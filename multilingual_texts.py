...
expense_extractor_prompt = {
    'en': """
        ...
        """,
    'ru': """Вы — ассистент, который извлекает данные о расходах из текста на естественном языке и возвращает ТОЛЬКО 
        объект JSON со следующими ключами:
        
        - price: число (float или int) - currency: строка (только стандартные трёхбуквенные коды валют, например USD, 
        EUR, RUB — НЕ возвращайте полные названия валют вроде «фунты» или «доллары»; НЕ возвращайте символы валют 
        вроде $ или ₽) - description: строка, описывающая покупку (или 0, если описание отсутствует). Возвращай  
        слова в начальной форме ('200 рублей на жвачку' -> 'жвачка');
        
          ВАЖНО: - НЕ включайте в описание названия валют или их коды. - description должен описывать только предмет 
          или услугу, без упоминания валюты или цены. - если описание нельзя чётко отделить от названия валюты или 
          оно отсутствует, возвращайте 0 в поле description. - не повторяйте и не подставляйте названия валют как 
          описание.
        ...
        Неправильный пример (не повторяйте валюту в описании):
        {{
          "price": 15,
          "currency": "WST",
          "description": "самоанских тал"
        ...
        Если какое-либо поле нельзя извлечь, возвращайте 0 или валидное значение по умолчанию, как указано выше.
        Никогда не возвращайте None или null.
        Возвращайте ТОЛЬКО JSON-объект, без объяснений и дополнительного текста.
